...
"""
mutable struct GD53 <: AbstractCVI
    dim::Int64
    n_samples::Int64
...
function setup!(cvi::GD53, sample::Array{T, 1}) where {T<:Real}
...
"""
    param_inc!(cvi::GD53, sample::Array{T, 1}, label::I) where {T<:Real, I<:Int}

Compute the Generalized Dunn's Index 53 (GD53) CVI incrementally.
"""
function param_inc!(cvi::GD53, sample::Array{T, 1}, label::I) where {T<:Real, I<:Int}
...
end # param_inc!(cvi::GD53, sample::Array{T, 1}, label::I) where {T<:Real, I<:Int}

"""
    param_batch!(cvi::GD53, data::Array{T, 2}, labels::Array{I, 1}) where {T<:Real, I<:Int}

Compute the Generalized Dunn's Index 53 (GD53) CVI in batch.
"""
function param_batch!(cvi::GD53, data::Array{T, 2}, labels::Array{I, 1}) where {T<:Real, I<:Int}
    ...
    for ix = 1 : (cvi.n_clusters - 1)
        for jx = ix + 1 : cvi.n_clusters
            ...
            cvi.D[jx, ix] = (cvi.CP[ix] + cvi.CP[jx]) / (cvi.n[ix] + cvi.n[jx])
        end
    end
    cvi.D = cvi.D + transpose(cvi.D)
end # param_batch(cvi::GD53, data::Array{Real, 2}, labels::Array{Real, 1})
